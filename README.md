...
With `y2v:jsonld-playground` the `content` value is the URL of a playground or JSON-LD viewer, in which `{jsonld}` is replaced by the URL encoded example (see the `example` key). For the [JSON-LD Playground](https://json-ld.org/playground), which takes its input from the URL fragment:

```html
<meta name="y2v:jsonld-playground" content="https://json-ld.org/playground#json-ld={jsonld}&amp;startTab=tab-expand"/>
```

and for [SoWasm](https://perso.liris.cnrs.fr/pierre-antoine.champin/2023/sowasm/), which takes it from the query string:
...
Whichever viewer is used, the button text always reads &ldquo;Open in JSON-LD Playground&rdquo;. The script only generates the markup for the button; placing it is up to the template's CSS, see [Styling the JSON-LD Playground button](#1311-styling-the-json-ld-playground-button) below.
...
The script generates the markup for the &ldquo;Open in JSON-LD Playground&rdquo; button, but not its presentation. Each example is wrapped as follows:
...
The colours, the sizes, and the corner used are a matter of taste and may be adapted to the surrounding style. Bear in mind that respec decorates `pre.example` with a title bar of its own, so the offsets may need some adjustment if the template styles its examples differently.
