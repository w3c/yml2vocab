    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                imported at the beginning of the generated context file. This is done by generating an array in the
                "@context" file, consisting of the array of the URL in this key, followed by the context object
        ...
        <tr>
            ...
            <td>
                ...
                official URL for the vocabulary. This makes the vocabulary easily extensible in JSON-LD, though possibly
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                If the template names a playground or JSON-LD viewer (see <a href="#131-html-templating">HTML Templating</a>), each example is
                followed, in the HTML version, by an &ldquo;Open in JSON-LD Playground&rdquo; button that opens the example in that viewer.
            </td>
            <td>No</td>
        </tr>
        <tr>
...
Some efforts are made to make the output files (HTML, JSON-LD, Turtle, or YAML-LD) properly formatted to make them readable. A subset of the [`editorconfig`](https://spec.editorconfig.org) facilities are also taken into account. Namely, if an `.editorconfig` file is found, the following supported pairs are used (with the default values in parenthesis):
...
The `reserved` and `deprecated` section are filled with terms that have been labelled as `reserved`, resp. `deprecated`, using the `status` key in the term's defining block. If no `reserved` or `deprecated` entries are defined, the respective sections will be removed from the resulting HTML. Similarly, if no alternate context files are specified, the `<ul id=contexts>` entry is removed.

Optional features of the script are switched on by `meta` elements whose name carries a `y2v:` prefix. These elements configure the generation and are removed from the generated document; if a setting is missing, the corresponding feature is off.

| `meta` name              | Value              | Effect                                                                              |
| :----------------------- | :----------------- | :---------------------------------------------------------------------------------- |
| `y2v:jsonld-playground`  | URL template       | Adds a button next to each example, opening it in the playground or viewer at that URL |
| `y2v:group-individuals`  | `true`             | Gathers the individuals into a subchapter per type, instead of listing them one after the other |

With `y2v:jsonld-playground` the `content` value is the URL of a playground or JSON-LD viewer, in which `{jsonld}` is replaced by the URL encoded example (see the `example` key).
As an example, to use the [JSON-LD Playground](https://json-ld.org/playground), the `meta` element may be:

```html
<meta name="y2v:jsonld-playground" content="https://json-ld.org/playground#json-ld={jsonld}&amp;startTab=tab-expand"/>
```

As another example, to use [SoWasm](https://perso.liris.cnrs.fr/pierre-antoine.champin/2023/sowasm/) with an immediate display of the Turtle version of the example, the `meta` element may be:

```html
<meta name="y2v:jsonld-playground" content="https://perso.liris.cnrs.fr/pierre-antoine.champin/2023/sowasm/?input={jsonld}&amp;oformat=text%2Fturtle&amp;webDocLoader="/>
```

Whichever viewer is used, the button text always reads “Open in JSON-LD Playground”. The script only generates the markup for the button; placing it is up to the template's CSS, see [Styling the JSON-LD Playground button](#1311-styling-the-json-ld-playground-button) below.

The `y2v:group-individuals` setting introduces, within the sections on individuals, a subsection per type (or per combination of types), listing the individuals that refer to it:

```html
<meta name="y2v:group-individuals" content="true"/>
```

Only the value `true` switches the grouping on; without the element the individuals follow one another, as all other terms do. The links to the individuals of a class, shown in the class sections, are generated either way.

#### 1.3.1.1. Styling the JSON-LD Playground button

The script generates the markup for the “Open in JSON-LD Playground” button, but not its presentation. Each example is wrapped as follows:

```html
<div class="example-with-playground">
  <pre class="example prettyprint language-json" title="…">…</pre>
  <a href="…" target="_blank" rel="noopener noreferrer" class="jsonld-playground-button">Open in JSON-LD Playground</a>
</div>
```

Left unstyled, the link simply trails the example block. The [example template](https://github.com/w3c/yml2vocab/tree/main/example/template.html) carries a rule set that positions the button to the right upper corner of the example, instead. The vocabulary authors are advised to copy something like it into their own template:

```css
.example-with-playground {
  position: relative;          /* the positioning context for the button */
}

a.jsonld-playground-button {
  position: absolute;          /* …so these offsets are relative to the example */
  top: 0.4em;
  right: 0.4em;
  z-index: 1;                  /* stays above the example's own decoration */
  padding: 0.3em 0.7em;
  font-size: 0.8em;
  font-weight: bold;
  color: #fff;
...
a.jsonld-playground-button:hover {
  background-color: #00457a;
}
```

The colours, the sizes, and the corner used may be adapted to the surrounding style.

If using [`respec`](https://respec.org) for the template, bear in mind that respec decorates `pre.example` with a title bar of its own, so the offsets may need some adjustment if the template styles its examples differently.
...
```sh
yml2vocab [-v vocab_file_name] [-t template_file_name] [-c] [-y]
```
...
```sh
deno run -A /a/b/c/main.ts [-v vocab_fname] [-t template_fname] [-c] [-y]
```
