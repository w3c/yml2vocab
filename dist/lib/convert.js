...
function getData(vocab_source) {
    ...
    const properties = (vocab.property !== undefined) ?
        vocab.property.map((raw) => {
            ...
            Object.assign(output, {
                ...
                scope: raw.scope?.map(val => factory_1.factory.class(val)),
                example: raw.example,
                known_as: raw.known_as,
                dataset: dataset,
                container: container,
    ...
    for (const current_class of classes) {
        if (current_class.subClassOf && current_class.subClassOf.length > 0) {
            for (const superclass of current_class.subClassOf) {
                const parent = classes.find((cl) => factory_1.RDFTermFactory.equals(cl, superclass));
                if (parent) {
    ...
    common_1.global.protected = vocab.json_ld?.protected ?? true;
    /********************************************************************************************/
